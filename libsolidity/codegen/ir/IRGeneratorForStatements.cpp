...
bool IRGeneratorForStatements::visit(Assignment const& _assignment)
{
	...
	{
		Type const* rightIntermediateType = type(_assignment.rightHandSide()).mobileType();
		...
		IRVariable newValue = IRVariable(m_context.newYulVariable(), type(_assignment.leftHandSide()));
		define(newValue) << shiftOperation(binaryOperator, leftIntermediate, value);

		writeToLValue(*m_currentLValue, newValue);
		m_currentLValue.reset();
		if (*_assignment.annotation().type != *TypeProvider::emptyTuple())
			define(_assignment, newValue);
	}
	else
	{
